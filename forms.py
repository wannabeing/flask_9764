...
class UserCreateForm(FlaskForm):
    ...
    password1 = PasswordField('비밀번호', validators=[
        DataRequired('입력하세요.'), EqualTo('password2', '비밀번호가 일치하지 않습니다')])
...
class UserLoginForm(FlaskForm):
    username = StringField('아이디', validators=[DataRequired(' 아이디를 정확히 입력하세요.'), Length(min=3, max=25)])
    password = PasswordField('비밀번호', validators=[DataRequired(' 비밀번호를 정확히 입력하세요.')])
